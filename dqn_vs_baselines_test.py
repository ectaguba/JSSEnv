...
Usage:
    python run_baseline_evaluation.py \
      ...
      --runs 5 \
      --dqn-model-path models/dqn_model.pth \
...
class QNetwork(nn.Module):
    ...
    def forward(self, real_obs: torch.Tensor):
        # real_obs: Tensor of shape (B, jobs, feat)
        B = real_obs.shape[0]
        x = real_obs.view(B, -1)  # flatten (B, jobs*feat)
        return self.net(x)
...
class BaselineEvaluator:
    def __init__(self, instances=None, num_runs=5, output_dir="evaluation_results",
        ...
        # If a DQN path is provided, load the network once upfront (after discovering obs/action shape)
    ...
    def _load_dqn_model(self):
        ...
        inst0_path = os.path.join("JSSEnv", "envs", "instances", inst0)
    ...
    def evaluate_dqn_on_instance(self, instance_id):
        """
        ...
        inst_path = os.path.join("JSSEnv", "envs", "instances", instance_id)
